...
class TestProfitIndicators:
    ...
    def create_test_row(
        self, asset="TEST", total_return_pct=0, actual_value_eur=100, unrealised_eur=0
    ):
    ...
    def test_exactly_one_euro_position_loss(self):
        ...
        indicator = self.get_profit_indicator(row)
        assert (
            indicator == "💥 Heavy Loss"
        )  # Should use normal indicators at exactly €1
...
class TestProfitIndicatorEdgeCases:
    ...
    def test_negative_zero(self):
        ...
        row = pd.Series(
            {"Total Return %": -0.0, "Actual Value €": 100, "Unrealised €": 0}
        )
        indicator = self.get_profit_indicator(row)
        assert indicator == "⚪ Break-even"
    ...
    def test_infinity_values(self):
        ...
        row = pd.Series(
            {"Total Return %": float("inf"), "Actual Value €": 100, "Unrealised €": 50}
        )
        indicator = self.get_profit_indicator(row)
        assert indicator == "🚀 Strong Profit"
    ...
    def test_nan_handling(self):
        ...
        row = pd.Series(
            {"Total Return %": np.nan, "Actual Value €": 100, "Unrealised €": 0}
        )
