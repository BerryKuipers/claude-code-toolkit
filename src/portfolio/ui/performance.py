...
class PerformanceOptimizer:
    ...
    def optimize_dataframe_display(
        df: pd.DataFrame, max_rows: int = 1000
    ) -> pd.DataFrame:
        ...
        if len(df) > max_rows:
            logger.warning(
                f"DataFrame has {len(df)} rows, truncating to {max_rows} for performance"
            )
            df = df.head(max_rows)
    ...
    def get_memory_usage() -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_entries": len(
                [k for k in st.session_state.keys() if "cache" in k.lower()]
            ),
            "lazy_components": len(
                [k for k in st.session_state.keys() if k.startswith("lazy_")]
            ),
        }
