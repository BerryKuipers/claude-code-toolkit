...
def _parse_price_override(override_str: str) -> Dict[str, Decimal]:
    ...
    for pair in override_str.split(","):
        ...
        if "=" not in pair:
            typer.echo(
                f"Invalid price override format '{pair}': must be ASSET=PRICE", err=True
            )
            raise typer.Exit(1)
        ...
        if not asset or not price_str:
            typer.echo(
                f"Invalid price override '{pair}': asset and price cannot be empty",
                err=True,
            )
            raise typer.Exit(1)
        ...
        try:
            ...
            typer.echo(
                f"Invalid price value '{price_str}' for {asset}: {exc}", err=True
            )
            raise typer.Exit(1)
...
def _get_bitvavo_client() -> Bitvavo:
    ...
    if not api_key or not api_secret:
        typer.echo(
            "Error: Set BITVAVO_API_KEY and BITVAVO_API_SECRET environment variables",
            err=True,
        )
        raise typer.Exit(1)
...
def report(
    ...
    override: Optional[str] = typer.Option(
        None, "--override", "-o", help="Override live prices (e.g., BTC=35000,ETH=1800)"
    ),
) -> None:
...
def whatif(
    ...
    if "=" not in price:
        typer.echo(
            "Error: Price must be in format ASSET=PRICE (e.g., BTC=35000)", err=True
        )
        raise typer.Exit(1)
