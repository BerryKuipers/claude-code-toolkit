...
class RiskAssessment:
    ...
    def assess_portfolio_risk(self, portfolio_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            positioned_assets = portfolio_data[
                portfolio_data["Actual Amount"] > 0
            ].copy()
            ...
            positioned_assets["Allocation %"] = (
                positioned_assets["Actual Value €"] / total_value * 100
            )
            ...
            risk_analysis = {
                ...
                "position_sizing_risk": self._assess_position_sizing_risk(
                    positioned_assets
                ),
    ...
    def _calculate_overall_risk_score(
        self, positioned_assets: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        extreme_positions = len(
            positioned_assets[positioned_assets["Total Return %"].abs() > 50]
        )
        extreme_risk = (
            extreme_positions / len(positioned_assets)
        ) * 25  # Max 25 points
    ...
    def _assess_concentration_risk(
        ...
        concentrated_positions = positioned_assets[
            positioned_assets["Allocation %"] > 20
        ]
        ...
        return {
            ...
            "concentrated_assets": (
                concentrated_positions["Asset"].tolist()
                if not concentrated_positions.empty
                else []
            ),
            "recommendation": self._get_concentration_recommendation(
                max_single_position, top_5_concentration
            ),
        }

    def _get_concentration_recommendation(
        self, max_position: float, top_5: float
    ) -> str:
        ...
        if max_position > 50:
            ...
            return (
                "Highly concentrated - consider rebalancing to reduce single-asset risk"
            )
    ...
    def _get_volatility_recommendation(self, risk_level: str, metrics: Dict) -> str:
        ...
        if risk_level == "VERY_HIGH":
            ...
            return (
                "Low volatility - stable portfolio, suitable for larger position sizes"
            )
    ...
    def _assess_loss_risk(self, positioned_assets: pd.DataFrame) -> Dict[str, Any]:
        ...
        significant_losses = positioned_assets[
            positioned_assets["Total Return %"] < -20
        ]
        ...
        total_loss_value = (
            losing_positions["Unrealised €"].sum() if not losing_positions.empty else 0
        )
        ...
        return {
            ...
            "worst_performer": (
                {
                    "asset": positioned_assets.loc[
                        positioned_assets["Total Return %"].idxmin(), "Asset"
                    ],
            ...
            "recommendation": self._get_loss_risk_recommendation(
                risk_level, loss_ratio
            ),
        }
    ...
    def _assess_liquidity_risk(self, positioned_assets: pd.DataFrame) -> Dict[str, Any]:
        ...
        liquid_positions = positioned_assets[
            positioned_assets["Asset"].isin(liquid_assets)
        ]
        illiquid_positions = positioned_assets[
            ~positioned_assets["Asset"].isin(liquid_assets)
        ]

        liquid_value_pct = (
            (
                liquid_positions["Actual Value €"].sum()
                / positioned_assets["Actual Value €"].sum()
                * 100
            )
            if not positioned_assets.empty
            else 0
        ...
        return {
            ...
            "major_liquid_assets": (
                liquid_positions["Asset"].tolist() if not liquid_positions.empty else []
            ),
            "potentially_illiquid": (
                illiquid_positions["Asset"].tolist()
                if not illiquid_positions.empty
                else []
            ),
            "recommendation": self._get_liquidity_recommendation(
                risk_level, liquid_value_pct
            ),
        }
    ...
    def _assess_correlation_risk(
        ...
        major_value = positioned_assets[positioned_assets["Asset"].isin(major_coins)][
            "Actual Value €"
        ].sum()
        alt_value = positioned_assets[positioned_assets["Asset"].isin(altcoins)][
            "Actual Value €"
        ].sum()
        meme_value = positioned_assets[positioned_assets["Asset"].isin(meme_coins)][
            "Actual Value €"
        ].sum()
        other_value = (
            positioned_assets["Actual Value €"].sum()
    ...
    def _get_correlation_recommendation(
        self, risk_level: str, distribution: Dict
    ) -> str:
        """Get recommendation for correlation risk."""
        if risk_level == "HIGH":
            return (
                "High correlation risk - portfolio concentrated in one asset category"
            )
        elif risk_level == "MEDIUM":
    ...
    def _assess_position_sizing_risk(
        self, positioned_assets: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        oversized_positions = len(
            positioned_assets[positioned_assets["Allocation %"] > 25]
        )
        undersized_positions = len(
            positioned_assets[positioned_assets["Allocation %"] < 2]
        )
        ...
        return {
            ...
            "recommendation": self._get_position_sizing_recommendation(
                risk_level, oversized_positions
            ),
        }

    def _get_position_sizing_recommendation(
        self, risk_level: str, oversized: int
    ) -> str:
    ...
    def _generate_risk_recommendations(
        ...
        if max_allocation > 40:
            recommendations.append(
                f"URGENT: Reduce largest position ({max_allocation:.1f}%) to under 25%"
            )
        ...
        if not major_losses.empty:
            recommendations.append(
                f"Review {len(major_losses)} positions with >30% losses - consider stop-loss strategy"
            )
        ...
        if not major_gains.empty:
            recommendations.append(
                f"Consider taking profits on {len(major_gains)} positions with >100% gains"
            )

        # Diversification recommendations
        if len(positioned_assets) < 5:
            recommendations.append(
                "Consider increasing diversification - portfolio has fewer than 5 positions"
            )
        elif len(positioned_assets) > 20:
            recommendations.append(
                "Consider consolidating positions - portfolio may be over-diversified"
            )
