...
class PredictionEngine:
    ...
    def get_comprehensive_analysis(
        self, portfolio_data: pd.DataFrame, asset: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def _analyze_single_asset(
        self, portfolio_data: pd.DataFrame, asset: str
    ) -> Dict[str, Any]:
        """Analyze a single asset comprehensively."""
        try:
            # Technical analysis
            technical_analysis = self.technical_analyzer.analyze_asset_technicals(
                asset, portfolio_data
            )
            ...
            comprehensive_analysis = {
                ...
                "summary": self._generate_asset_summary(
                    asset, technical_analysis, asset_risk, predictions
                ),
                "action_recommendations": self._generate_asset_recommendations(
                    asset_row, technical_analysis, asset_risk
                ),
            }
    ...
    def _analyze_portfolio(self, portfolio_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            portfolio_technical = (
                self.technical_analyzer.get_portfolio_technical_summary(portfolio_data)
            )
            ...
            portfolio_predictions = self._generate_portfolio_predictions(
                portfolio_data, portfolio_technical, portfolio_risk
            )
            ...
            comprehensive_analysis = {
                ...
                "portfolio_summary": self._generate_portfolio_summary(
                    portfolio_technical, portfolio_risk
                ),
    ...
    def _assess_single_asset_risk(self, asset_row: pd.Series) -> Dict[str, Any]:
        ...
        allocation_pct = (
            float(asset_row.get("Allocation %", 0))
            if "Allocation %" in asset_row
            else 0
        )
        ...
        return {
            ...
            "recommendation": self._get_single_asset_risk_recommendation(
                risk_level, return_pct
            ),
        }

    def _get_single_asset_risk_recommendation(
        self, risk_level: str, return_pct: float
    ) -> str:
        ...
        if risk_level == "VERY_HIGH":
            if return_pct > 0:
                ...
                return (
                    "Very high risk with losses - implement immediate risk management"
                )
    ...
    def _generate_asset_predictions(
        self, asset_row: pd.Series, technical_analysis: Dict
    ) -> Dict[str, Any]:
        ...
        momentum = technical_analysis.get("momentum_indicators", {}).get(
            "momentum", "NEUTRAL"
        )

        # Price predictions (simple heuristic-based)
        predictions = {
            "short_term_outlook": self._predict_short_term_outlook(
                signal, momentum, return_pct
            ),
            "support_resistance": technical_analysis.get("support_resistance", {}),
            "price_targets": self._calculate_price_targets(
                current_price, return_pct, signal
            ),
    ...
    def _predict_short_term_outlook(
        self, signal: str, momentum: str, return_pct: float
    ) -> Dict[str, Any]:
        ...
        elif momentum in ["VERY_WEAK", "NEGATIVE"] and outlook in [
            "NEUTRAL",
            "BEARISH",
        ]:
        ...
        return {
            ...
            "signal": signal,
            "momentum": momentum,
            "reasoning": self._explain_outlook_reasoning(
                outlook, signal, momentum, return_pct
            ),
        }

    def _explain_outlook_reasoning(
        self, outlook: str, signal: str, momentum: str, return_pct: float
    ) -> str:
    ...
    def _calculate_price_targets(
        self, current_price: float, return_pct: float, signal: str
    ) -> Dict[str, float]:
    ...
    def _calculate_risk_reward_ratio(
        self, current_price: float, technical_analysis: Dict
    ) -> Dict[str, Any]:
    ...
    def _generate_portfolio_predictions(
        self, portfolio_data: pd.DataFrame, technical: Dict, risk: Dict
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "expected_volatility": risk.get("overall_risk_score", {}).get(
                "risk_level", "UNKNOWN"
            ),
            "diversification_benefit": technical.get("diversification_score", 0),
            "rebalancing_needed": risk.get("concentration_risk", {}).get("risk_level")
            in ["HIGH", "VERY_HIGH"],
            "key_risks": risk.get("risk_recommendations", [])[:3],
            "growth_potential": (
                "HIGH" if avg_return > 15 else "MEDIUM" if avg_return > 5 else "LOW"
            ),
        }
    ...
    def _analyze_top_assets(self, positioned_assets: pd.DataFrame) -> Dict[str, Any]:
        ...
        by_value = positioned_assets.sort_values(
            "Actual Value €", ascending=False
        ).head(5)
    ...
    def _generate_asset_summary(
        self, asset: str, technical: Dict, risk: Dict, predictions: Dict
    ) -> str:
    ...
    def _generate_asset_recommendations(
        self, asset_row: pd.Series, technical: Dict, risk: Dict
    ) -> List[str]:
        ...
        if signal in ["STRONG_SELL", "SELL"] and return_pct > 50:
            recommendations.append(
                "Consider taking profits - strong sell signal with high gains"
            )
        elif signal in ["STRONG_BUY", "BUY"] and return_pct < -20:
            recommendations.append(
                "Potential buying opportunity - buy signal with significant discount"
            )

        if risk_level in ["VERY_HIGH", "HIGH"]:
            recommendations.append(
                "Implement strict risk management - high risk position"
            )
        ...
        if return_pct > 100:
            ...
            recommendations.append(
                "Significant losses - reassess position and consider stop-loss"
            )

        return recommendations[:5]

    def _generate_portfolio_recommendations(
        self, portfolio_data: pd.DataFrame, risk: Dict
    ) -> List[str]:
        ...
        if not positioned_assets.empty:
            ...
            if avg_return > 30:
                recommendations.append(
                    "Strong portfolio performance - consider systematic profit taking"
                )
            elif avg_return < -15:
                recommendations.append(
                    "Portfolio underperforming - review strategy and risk management"
                )
