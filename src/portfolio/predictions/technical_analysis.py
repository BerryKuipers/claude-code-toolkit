...
class TechnicalAnalyzer:
    ...
    def analyze_asset_technicals(self, asset: str, portfolio_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                "asset": asset,
                "current_price_eur": float(asset_row.get("Current Price €", 0)),
                "position_size": float(asset_row.get("Actual Amount", 0)),
                "cost_basis": float(asset_row.get("Cost €", 0)),
                "unrealized_pnl": float(asset_row.get("Unrealised €", 0)),
                "return_percentage": float(asset_row.get("Total Return %", 0)),
    ...
    def _generate_technical_signals(self, asset_row: pd.Series) -> Dict[str, Any]:
        ...
        cost_basis = float(asset_row.get("Cost €", 0))
        actual_amount = float(asset_row.get("Actual Amount", 0))
        ...
        return {
            ...
            "signals": signals,
            "price_vs_cost": round((current_price / avg_cost_per_unit - 1) * 100, 2) if avg_cost_per_unit > 0 else 0
        }
    ...
    def _calculate_support_resistance(self, asset_row: pd.Series) -> Dict[str, Any]:
        ...
        # Find nearest levels
        nearest_support = max([level for level in support_levels if level < current_price], default=support_levels[-1])
        nearest_resistance = min([level for level in resistance_levels if level > current_price], default=resistance_levels[-1])
    ...
    def _analyze_volatility(self, asset_row: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": self._get_volatility_recommendation(volatility_level, return_pct)
        }
    
    def _get_volatility_recommendation(self, volatility_level: str, return_pct: float) -> str:
    ...
    def _analyze_trend(self, asset_row: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            "trend_direction": "UP" if return_pct > 0 else "DOWN" if return_pct < 0 else "SIDEWAYS",
    ...
    def get_portfolio_technical_summary(self, portfolio_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            strong_performers = len(positioned_assets[positioned_assets["Total Return %"] > 20])
            weak_performers = len(positioned_assets[positioned_assets["Total Return %"] < -10])
            
            # Risk assessment
            high_risk_positions = len(positioned_assets[positioned_assets["Total Return %"].abs() > 30])
            ...
            return {
                ...
                "portfolio_momentum": "POSITIVE" if avg_return > 5 else "NEGATIVE" if avg_return < -5 else "NEUTRAL",
                "diversification_score": min(len(positioned_assets) / 10 * 100, 100),  # Simple diversification metric
                "risk_level": "HIGH" if high_risk_positions > len(positioned_assets) * 0.3 else "MEDIUM" if high_risk_positions > 0 else "LOW"
            }
