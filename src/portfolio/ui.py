...
def main():
    ...
    with col1:
        ...
        total_return = (
            ((totals["Total Value"] + totals["Total Realised"]) - total_invested)
            / total_invested
            * 100
            if total_invested > 0
            else 0
    ...
    with col2:
        ...
        if not df.empty:
            ...
            st.success(
                f"🏆 Best: {best_performer['Asset']} ({best_performer['Total Return %']:.2f}%)"
            )
            st.error(
                f"📉 Worst: {worst_performer['Asset']} ({worst_performer['Total Return %']:.2f}%)"
            )
