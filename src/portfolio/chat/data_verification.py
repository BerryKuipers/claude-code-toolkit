...
class PortfolioDataVerifier:
    ...
    def verify_function_result(self, function_name: str, function_args: str, result: str) -> Tuple[bool, str, Optional[str]]:
    ...
    def _verify_portfolio_summary(self, result_data: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
        ...
        reported_value = result_data.get("total_value_eur", 0)
        reported_cost = result_data.get("total_cost_eur", 0)
        reported_unrealised = result_data.get("total_unrealised_eur", 0)
    ...
    def _verify_asset_performance(self, args: Dict[str, Any], result_data: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
    ...
    def _verify_top_performers(self, args: Dict[str, Any], result_data: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
        ...
        portfolio_with_positions = self.portfolio_data[self.portfolio_data["Actual Amount"] > 0].copy()
        if portfolio_with_positions.empty:
            return True, json.dumps(result_data), None
        
        ascending = (performance_type == "worst")
        actual_top = portfolio_with_positions.sort_values("Total Return %", ascending=ascending).head(limit)
        ...
        for i, performer in enumerate(performers):
            ...
            if actual_asset["Asset"] != reported_asset:
                ...
                logger.warning(error_msg)  # Warning, not error, as order might vary slightly
        
        return True, json.dumps(result_data), None
    
    def _verify_coin_position(self, args: Dict[str, Any], result_data: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
        ...
        reported_amount = summary_data.get("actual_amount", 0)
        reported_value = summary_data.get("actual_value_eur", 0)
        reported_unrealised = summary_data.get("unrealised_eur", 0)
        reported_return_pct = summary_data.get("total_return_pct", 0)
        
        if abs(actual_amount - reported_amount) > 0.000001:  # Very small tolerance for crypto amounts
            error_msg = f"{asset} amount mismatch: actual {actual_amount:.6f} vs reported {reported_amount:.6f}"
            logger.error(error_msg)
            return False, json.dumps(result_data), error_msg
    ...
    def _verify_portfolio_allocation(self, args: Dict[str, Any], result_data: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
    ...
    def _verify_transfer_analysis(self, args: Dict[str, Any], result_data: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
    ...
    def get_verification_stats(self) -> Dict[str, Any]:
        ...
        verified_count = sum(1 for log in self.verification_log if log["status"] == "verified")
        trusted_count = sum(1 for log in self.verification_log if log["status"] == "trusted")
