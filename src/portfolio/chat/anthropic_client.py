    ...
    def chat_completion(
        ...
        try:
            ...
            if hasattr(response, "usage") and response.usage:
                ...
                logger.info(
                    f"Claude API usage - Input: {input_tokens}, "
            ...
            if "529" in error_msg or "overloaded" in error_msg.lower():
                raise Exception(
                    "🚨 Anthropic servers are currently overloaded. Please try again in a few minutes or switch to OpenAI GPT-4 in the model selector."
                )
            elif "401" in error_msg or "authentication" in error_msg.lower():
                raise Exception(
                    "❌ Authentication failed. Please check your Anthropic API key in the .env file."
                )
