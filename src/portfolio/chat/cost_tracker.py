...
from .base_llm_client import LLMProvider, AVAILABLE_MODELS
...
class CostTracker:
    ...
    def track_usage(
        ...
        st.session_state.total_llm_cost = getattr(st.session_state, 'total_llm_cost', 0.0) + cost
        st.session_state.total_llm_tokens = getattr(st.session_state, 'total_llm_tokens', 0) + prompt_tokens + completion_tokens
        st.session_state.total_llm_queries = getattr(st.session_state, 'total_llm_queries', 0) + 1
        
        logger.info(f"LLM usage tracked: {provider.value} {model_id} - {prompt_tokens}+{completion_tokens} tokens, ${cost:.4f}")
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_cost_per_query": self.total_session_cost / self.query_count if self.query_count > 0 else 0.0,
            "avg_tokens_per_query": self.total_session_tokens / self.query_count if self.query_count > 0 else 0.0,
