...
class PromptEditor:
    ...
    def _render_system_prompts_editor(self):
        ...
        st.markdown("*These prompts define the AI's core behavior and knowledge about your portfolio.*")
        ...
        current_context_prompt = st.session_state.custom_prompts.get("portfolio_context", "")
        ...
        current_function_prompt = st.session_state.custom_prompts.get("function_instructions", "")
        ...
        if new_function_prompt != current_function_prompt:
            st.session_state.custom_prompts["function_instructions"] = new_function_prompt
    ...
    def _render_personality_editor(self):
        ...
        with col1:
            # Communication style
            current_style = st.session_state.custom_prompts.get("communication_style", "professional")
            
            communication_style = st.selectbox(
                "Communication Style",
                options=["professional", "friendly", "casual", "technical", "enthusiastic"],
                index=["professional", "friendly", "casual", "technical", "enthusiastic"].index(current_style),
            ...
            current_risk_stance = st.session_state.custom_prompts.get("risk_stance", "balanced")
            
            risk_stance = st.selectbox(
                "Risk Advice Stance",
                options=["conservative", "balanced", "aggressive", "neutral"],
                index=["conservative", "balanced", "aggressive", "neutral"].index(current_risk_stance),
        ...
        with col2:
            # Explanation depth
            current_depth = st.session_state.custom_prompts.get("explanation_depth", "detailed")
            
            explanation_depth = st.selectbox(
                "Explanation Depth",
                options=["brief", "moderate", "detailed", "comprehensive"],
                index=["brief", "moderate", "detailed", "comprehensive"].index(current_depth),
    ...
    def _render_templates_editor(self):
        ...
        for template_key, template_name in template_types.items():
            current_template = st.session_state.custom_prompts.get(f"template_{template_key}", "")
    ...
    def _render_settings_editor(self):
        ...
        with st.expander("🔍 Current Configuration Summary"):
            config_summary = {
                "Communication Style": st.session_state.custom_prompts.get("communication_style", "professional"),
                "Risk Stance": st.session_state.custom_prompts.get("risk_stance", "balanced"),
                "Explanation Depth": st.session_state.custom_prompts.get("explanation_depth", "detailed"),
                "Use Emojis": st.session_state.custom_prompts.get("use_emojis", True),
                "Custom Traits": bool(st.session_state.custom_prompts.get("personality_traits", "")),
                "System Prompt Length": len(st.session_state.custom_prompts.get("system_prompt", "")),
                "Templates Configured": sum(1 for k in st.session_state.custom_prompts.keys() if k.startswith("template_"))
            }
...
Always call relevant functions to get current data rather than making assumptions.""",
    ...
    def _build_personality_prompt(self) -> str:
        ...
        if prompts.get("use_emojis", True):
            personality_parts.append("Use relevant emojis to make responses more engaging and easier to read.")
