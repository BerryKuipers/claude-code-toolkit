...
class PurchaseLotResponse(BaseModel):
    ...
    amount: Decimal = Field(
        ..., description="Crypto units in this lot", decimal_places=8
    )
    cost_eur: Decimal = Field(
        ..., description="Total EUR cost including fees", decimal_places=2
    )
    timestamp: int = Field(..., description="Milliseconds since epoch")

    @field_validator("amount", "cost_eur")
    @classmethod
...
class TransferSummaryResponse(BaseModel):
    ...
    total_deposits: Decimal = Field(
        ..., description="Total amount deposited", decimal_places=8
    )
    total_withdrawals: Decimal = Field(
        ..., description="Total amount withdrawn", decimal_places=8
    )
    net_transfers: Decimal = Field(
        ..., description="Net transfer amount", decimal_places=8
    )
    deposit_count: int = Field(..., description="Number of deposit transactions", ge=0)
    withdrawal_count: int = Field(
        ..., description="Number of withdrawal transactions", ge=0
    )
    potential_rewards: Decimal = Field(
        ..., description="Estimated staking rewards/airdrops", decimal_places=8
    )
...
class HoldingResponse(BaseModel):
    ...
    value_eur: Decimal = Field(
        ..., description="Current value in EUR", decimal_places=2
    )
    cost_basis: Decimal = Field(
        ..., description="Average cost basis in EUR", decimal_places=2
    )
    unrealized_pnl: Decimal = Field(
        ..., description="Unrealized P&L in EUR", decimal_places=2
    )
    realized_pnl: Decimal = Field(
        ..., description="Realized P&L in EUR", decimal_places=2
    )
    portfolio_percentage: Decimal = Field(
        ..., description="Portfolio allocation %", decimal_places=2
    )
    total_return_percentage: Decimal = Field(
        ..., description="Total return %", decimal_places=2
    )

    @field_validator("quantity", "current_price", "value_eur", "cost_basis")
    @classmethod
...
class TransactionResponse(BaseModel):
    ...
    price: Decimal = Field(
        ..., description="Transaction price in EUR", decimal_places=4
    )
...
class PortfolioSummaryResponse(BaseResponse):
    ...
    total_value: Decimal = Field(
        ..., description="Total portfolio value in EUR", decimal_places=2
    )
    total_cost: Decimal = Field(
        ..., description="Total cost basis in EUR", decimal_places=2
    )
    realized_pnl: Decimal = Field(
        ..., description="Realized P&L in EUR", decimal_places=2
    )
    ...
    total_return_percentage: Decimal = Field(
        ..., description="Total return as percentage", decimal_places=2
    )
    asset_count: int = Field(..., description="Number of different assets held", ge=0)
    last_updated: datetime = Field(..., description="Last update timestamp")

    @field_validator("total_value", "total_cost")
    @classmethod
...
class ReconciliationResponse(BaseResponse):
    ...
    asset: AssetSymbol = Field(..., description="Asset symbol")
    fifo_amount: Decimal = Field(
        ..., description="FIFO calculated amount", decimal_places=8
    )
    actual_amount: Decimal = Field(
        ..., description="Actual balance amount", decimal_places=8
    )
    discrepancy: Decimal = Field(
        ..., description="Difference between FIFO and actual", decimal_places=8
    )
    transfer_summary: TransferSummaryResponse = Field(
